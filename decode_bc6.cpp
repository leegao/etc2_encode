...
struct PushConstants {
    uint32_t srcFormat;        // VK_FORMAT_BC6H_UFLOAT_BLOCK = 143 or VK_FORMAT_BC6H_SFLOAT_BLOCK = 144
    uint32_t srcRowLength;     // Source row stride in pixels
    uint32_t srcImageHeight;   // Source image height in pixels
    int32_t  offsetx;          // Destination X offset
    int32_t  offsety;          // Destination Y offset
    uint32_t dst_width;        // Destination width
    uint32_t dst_height;       // Destination height
    uint32_t srcBufferSize;    // Size of source buffer in bytes
...
#define COMPUTE_SPIRV_SIZE sizeof(COMPUTE_SPIRV)
...
bool WriteBC6HToKTX(const std::string& filename, 
                    uint32_t width, 
                    uint32_t height, 
                    const std::vector<uint8_t>& etc2Data) 
{
    ktxTextureCreateInfo createInfo;
    createInfo.glInternalformat = 0x8E8F; //  GL_COMPRESSED_RGB_BPTC_UNSIGNED_FLOAT
    ...
    result = ktxTexture_SetImageFromMemory(ktxTexture(texture), 0, 0, 0, (const unsigned char*) etc2Data.data(), etc2Data.size());
    if (result != KTX_SUCCESS) {
...
bool WriteSFLOATToKTX(const std::string& filename, 
                    uint32_t width, 
                    uint32_t height, 
                    const std::vector<uint8_t>& etc2Data) 
{
    ktxTextureCreateInfo createInfo;
    createInfo.glInternalformat = 0x881A; //  GL_RGBA16F
...
std::vector<uint8_t> LoadRawBC6H(const std::string& filename, int width, int height) {
    int blocksX = (width + 3) / 4;
    int blocksY = (height + 3) / 4;
    size_t expectedSize = blocksX * blocksY * 16; // 16 bytes per BC6H block
    
    std::ifstream file(filename, std::ios::binary | std::ios::ate);
    if (!file.is_open()) {
        throw std::runtime_error("Failed to open BC6H file: " + filename);
    }
    
    size_t fileSize = file.tellg();
    if (fileSize != expectedSize) {
        throw std::runtime_error("BC6H file size mismatch. Expected: " + 
...
std::vector<uint8_t> LoadRawRGBA16_SFLOAT(const std::string& filename, int width, int height) {
...
int main() {
    int width = 128, height = 128, channels;
    auto bc6hData = LoadRawBC6H("205.bc6h", width, height);
    std::cout << "Loaded BC6H data: " << bc6hData.size() << " bytes" << std::endl;
    
    int blocksX = (width + 3) / 4;
    int blocksY = (height + 3) / 4;
    size_t totalBlocks = static_cast<size_t>(blocksX) * blocksY;
    size_t decodedBufferSize = width * height * 4;
    ...
    size_t stagingBufferSize = bc6hData.size();
    ...
    // Load the bc6h data into staging buffer
    void* mappedData;
    vkMapMemory(device, stagingMemory, 0, stagingBufferSize, 0, &mappedData);
    std::memcpy(mappedData, bc6hData.data(), stagingBufferSize);
    vkUnmapMemory(device, stagingMemory);

    VkBuffer decodedBuffer;
    VkBufferCreateInfo compBufInfo{ VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO, nullptr, 0, decodedBufferSize, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_SRC_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT };
    VK_CHECK(vkCreateBuffer(device, &compBufInfo, nullptr, &decodedBuffer));
    VkMemoryRequirements compReqs;
    vkGetBufferMemoryRequirements(device, decodedBuffer, &compReqs);
    ...
    vkBindBufferMemory(device, decodedBuffer, compressedMemory, 0);
    ...
    bufferDescInfo.buffer = decodedBuffer;
    bufferDescInfo.offset = 0;
    bufferDescInfo.range = decodedBufferSize;
    ...
    constants.srcFormat = 143;
    constants.srcRowLength = width;
    constants.srcImageHeight = height;
    constants.dst_width = width;
    constants.dst_height = height;
    constants.srcBufferSize = bc6hData.size();
    ...
    VkBufferMemoryBarrier compFinishBarrier{ VK_STRUCTURE_TYPE_BUFFER_MEMORY_BARRIER, nullptr, VK_ACCESS_SHADER_WRITE_BIT, VK_ACCESS_HOST_READ_BIT, queueFamilyIndex, queueFamilyIndex, decodedBuffer, 0, decodedBufferSize };
    ...
    std::vector<uint8_t> decodedBlocks(decodedBufferSize);
    vkMapMemory(device, compressedMemory, 0, decodedBufferSize, 0, &mappedData);
    std::memcpy(decodedBlocks.data(), mappedData, decodedBufferSize);
    ...
    //                     std::cout << decodedBlocks[dstIdx + 0] << "\n";
    //                 }
    //             }
    //         }
    //     }
    ...
    //     for (int bx = 0; bx < blocksX; ++bx) {
    //         size_t blockIdx = by * blocksX + bx;
    //         const uint8_t* blockData = decodedBlocks[blockIdx].reconstructed;
    ...
    if (stbi_write_png("reconstructed_bc6.png", width, height, 4, decodedBlocks.data(), width * 4)) {
    ...
    // if (outFile.is_open()) {
    //     outFile.write(reinterpret_cast<const char*>(encodedBlocks.data()), decodedBufferSize);
    //     outFile.close();
    //     std::cout << "Wrote the raw etc2 ssbo to output.etc2" << std::endl;
    // }

    WriteBC6HToKTX("bc6.ktx", width, height, bc6hData);
    auto i205_sfloat = LoadRawRGBA16_SFLOAT("205.sfloat", width, height);
    WriteSFLOATToKTX("sfloat.ktx", width, height, i205_sfloat);
    return 0;
}
